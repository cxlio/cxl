import { Observable, Subject, Item, Event } from '../rx';
import { dom } from '../dom';
...
class Renderer {
	...
	request() {
		if (this.raf === null) this.raf = requestAnimationFrame(this.commit);
	}
...
class Directive {
	constructor(public element, public parameter, public owner) {
...
class View<T> extends StoreBase<T> {
	constructor(State: T, host: HtmlElement) {
		super(State);
		this.host = host;
...
class Template {
	...
	compile(owner) {
		...
		if (nodes.childNodes.length) compiler.traverse(nodes.firstChild, owner);
...
directive('list', {
	...
	update(event) {
		if (event.type === 'added') this.list.add(event.value, event.nextValue);
...
directive('content', {
	...
	connect() {
		if (this.parameter) {
			...
			for (var node of this.owner.host.childNodes) this.assignSlot(node);
		}
	},
...
connectedSources({
	...
	keypress(el, param, view, subscriber) {
		...
		return new EventListener(el, 'keydown', ev => {
			if (!param || ev.key.toLowerCase() === param) subscriber.next(ev);
		});
	},
...
pipes({
	attribute(val) {
		return cxl.dom.setAttribute(this.element, this.parameter || val, val);
	},
