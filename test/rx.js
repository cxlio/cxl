...
(() => {
	...
	QUnit.test('Observable#constructor', function(a) {
		var observable = new cxl.rx.Observable(function subscribe(observer) {
				observer.next(1);
				observer.next(2);
				observer.next(3);
				observer.complete();
	...
	QUnit.test('Observable#subscribe', function(a) {
		var o = new cxl.rx.Observable(function(s) {
				s.next();
				s.complete();
			}),
