...
(() => {
	...
	function testFocus(c, a) {
		...
		a.equal(getComputedStyle(c).pointerEvents, 'none');
	}
	...
	function testComponent(name, a) {
		...
		if (c.hasAttribute('tabindex') && c.tabIndex !== -1) testFocus(c, a);

		if (def.attributes) {
			testAttributes(c, def, a);
			if (def.attributes.indexOf('value') !== -1) testValue(c, a);
		}
...
QUnit.test('cxl-select - preset value', function(a) {
	var A = $$compile(`<cxl-select value="two">
...
QUnit.test('cxl-checkbox - checked true-value', function(a) {
	var A = $$compile(
		'<cxl-checkbox &="@value:=test" checked true-value="yes">'
	);
...
QUnit.test('cxl-checkbox - value sets checked', function(a) {
	var A = $$compile(
		'<cxl-checkbox &="id(c) @checked:=test" value="yes" true-value="yes">'
	);
	a.equal(A.state.test, true);
});

QUnit.test('cxl-switch', function(a) {
	var A = cxl.dom('cxl-switch');
	a.ok(A);
